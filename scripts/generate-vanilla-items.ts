/**
 * Generate Vanilla Terraria items
 *
 * This script generates curated vanilla Terraria items with proper IDs (1-99,999 range)
 *
 * Run with: pnpm tsx scripts/generate-vanilla-items.ts
 */

import * as fs from "fs"
import * as path from "path"

const DATA_DIR = path.join(process.cwd(), "data")

interface TerrariaItem {
    id: number
    name: string
    mod: string
    type: "weapon" | "armor" | "accessory" | "tool"
    rarity: number
    defense?: number
    damage?: number
    armorType?: "helmet" | "chestplate" | "leggings"
    weaponClass?: "melee" | "ranged" | "magic" | "summoner"
}

function getVanillaItems(): TerrariaItem[] {
    console.log("🎮 Generating Vanilla Terraria items...")

    // Essential vanilla items for loadout building
    const items: TerrariaItem[] = [
        // Pre-Hardmode Melee Weapons
        {
            id: 1,
            name: "Copper Shortsword",
            mod: "vanilla",
            type: "weapon",
            rarity: 0,
            damage: 5,
            weaponClass: "melee",
        },
        {
            id: 24,
            name: "Wooden Sword",
            mod: "vanilla",
            type: "weapon",
            rarity: 0,
            damage: 7,
            weaponClass: "melee",
        },
        {
            id: 3507,
            name: "Starfury",
            mod: "vanilla",
            type: "weapon",
            rarity: 1,
            damage: 22,
            weaponClass: "melee",
        },
        {
            id: 65,
            name: "Blade of Grass",
            mod: "vanilla",
            type: "weapon",
            rarity: 2,
            damage: 28,
            weaponClass: "melee",
        },
        {
            id: 273,
            name: "Fiery Greatsword",
            mod: "vanilla",
            type: "weapon",
            rarity: 3,
            damage: 36,
            weaponClass: "melee",
        },
        {
            id: 795,
            name: "Nights Edge",
            mod: "vanilla",
            type: "weapon",
            rarity: 3,
            damage: 42,
            weaponClass: "melee",
        },

        // Pre-Hardmode Ranged Weapons
        {
            id: 96,
            name: "Wooden Bow",
            mod: "vanilla",
            type: "weapon",
            rarity: 0,
            damage: 4,
            weaponClass: "ranged",
        },
        {
            id: 98,
            name: "Platinum Bow",
            mod: "vanilla",
            type: "weapon",
            rarity: 0,
            damage: 13,
            weaponClass: "ranged",
        },
        {
            id: 120,
            name: "Flintlock Pistol",
            mod: "vanilla",
            type: "weapon",
            rarity: 0,
            damage: 10,
            weaponClass: "ranged",
        },
        {
            id: 164,
            name: "Musket",
            mod: "vanilla",
            type: "weapon",
            rarity: 1,
            damage: 31,
            weaponClass: "ranged",
        },
        {
            id: 800,
            name: "The Undertaker",
            mod: "vanilla",
            type: "weapon",
            rarity: 1,
            damage: 14,
            weaponClass: "ranged",
        },
        {
            id: 162,
            name: "Minishark",
            mod: "vanilla",
            type: "weapon",
            rarity: 1,
            damage: 6,
            weaponClass: "ranged",
        },
        {
            id: 122,
            name: "Boomstick",
            mod: "vanilla",
            type: "weapon",
            rarity: 2,
            damage: 16,
            weaponClass: "ranged",
        },
        {
            id: 159,
            name: "Star Cannon",
            mod: "vanilla",
            type: "weapon",
            rarity: 2,
            damage: 60,
            weaponClass: "ranged",
        },
        {
            id: 3212,
            name: "Phoenix Blaster",
            mod: "vanilla",
            type: "weapon",
            rarity: 3,
            damage: 24,
            weaponClass: "ranged",
        },
        {
            id: 514,
            name: "Molten Fury",
            mod: "vanilla",
            type: "weapon",
            rarity: 3,
            damage: 31,
            weaponClass: "ranged",
        },
        {
            id: 3281,
            name: "Hellwing Bow",
            mod: "vanilla",
            type: "weapon",
            rarity: 3,
            damage: 20,
            weaponClass: "ranged",
        },

        // Pre-Hardmode Magic Weapons
        {
            id: 65,
            name: "Wand of Sparking",
            mod: "vanilla",
            type: "weapon",
            rarity: 0,
            damage: 8,
            weaponClass: "magic",
        },
        {
            id: 218,
            name: "Water Bolt",
            mod: "vanilla",
            type: "weapon",
            rarity: 1,
            damage: 17,
            weaponClass: "magic",
        },
        {
            id: 165,
            name: "Space Gun",
            mod: "vanilla",
            type: "weapon",
            rarity: 2,
            damage: 17,
            weaponClass: "magic",
        },
        {
            id: 114,
            name: "Flamelash",
            mod: "vanilla",
            type: "weapon",
            rarity: 3,
            damage: 34,
            weaponClass: "magic",
        },
        {
            id: 112,
            name: "Demon Scythe",
            mod: "vanilla",
            type: "weapon",
            rarity: 3,
            damage: 35,
            weaponClass: "magic",
        },

        // Pre-Hardmode Summoner Weapons
        {
            id: 3269,
            name: "Finch Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 0,
            damage: 6,
            weaponClass: "summoner",
        },
        {
            id: 4607,
            name: "Flinx Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 0,
            damage: 8,
            weaponClass: "summoner",
        },
        {
            id: 1132,
            name: "Slime Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 1,
            damage: 8,
            weaponClass: "summoner",
        },
        {
            id: 3269,
            name: "Vampire Frog Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 1,
            damage: 11,
            weaponClass: "summoner",
        },
        {
            id: 1802,
            name: "Imp Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 3,
            damage: 17,
            weaponClass: "summoner",
        },

        // Hardmode Melee Weapons
        {
            id: 426,
            name: "Excalibur",
            mod: "vanilla",
            type: "weapon",
            rarity: 4,
            damage: 66,
            weaponClass: "melee",
        },
        {
            id: 671,
            name: "True Excalibur",
            mod: "vanilla",
            type: "weapon",
            rarity: 5,
            damage: 70,
            weaponClass: "melee",
        },
        {
            id: 757,
            name: "Terra Blade",
            mod: "vanilla",
            type: "weapon",
            rarity: 8,
            damage: 95,
            weaponClass: "melee",
        },
        {
            id: 723,
            name: "Vampire Knives",
            mod: "vanilla",
            type: "weapon",
            rarity: 8,
            damage: 29,
            weaponClass: "melee",
        },
        {
            id: 3063,
            name: "Solar Eruption",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 105,
            weaponClass: "melee",
        },
        {
            id: 3389,
            name: "Meowmere",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 200,
            weaponClass: "melee",
        },
        {
            id: 3542,
            name: "Star Wrath",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 110,
            weaponClass: "melee",
        },
        {
            id: 3065,
            name: "Daybreak",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 150,
            weaponClass: "melee",
        },
        {
            id: 3823,
            name: "Zenith",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 190,
            weaponClass: "melee",
        },

        // Hardmode Ranged Weapons
        {
            id: 435,
            name: "Clockwork Assault Rifle",
            mod: "vanilla",
            type: "weapon",
            rarity: 4,
            damage: 19,
            weaponClass: "ranged",
        },
        {
            id: 484,
            name: "Megashark",
            mod: "vanilla",
            type: "weapon",
            rarity: 5,
            damage: 25,
            weaponClass: "ranged",
        },
        {
            id: 779,
            name: "Uzi",
            mod: "vanilla",
            type: "weapon",
            rarity: 5,
            damage: 30,
            weaponClass: "ranged",
        },
        {
            id: 534,
            name: "Daedalus Stormbow",
            mod: "vanilla",
            type: "weapon",
            rarity: 5,
            damage: 43,
            weaponClass: "ranged",
        },
        {
            id: 1265,
            name: "Chain Gun",
            mod: "vanilla",
            type: "weapon",
            rarity: 7,
            damage: 31,
            weaponClass: "ranged",
        },
        {
            id: 3546,
            name: "Tsunami",
            mod: "vanilla",
            type: "weapon",
            rarity: 8,
            damage: 53,
            weaponClass: "ranged",
        },
        {
            id: 1553,
            name: "SDMG",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 85,
            weaponClass: "ranged",
        },
        {
            id: 3065,
            name: "Celebration Mk2",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 120,
            weaponClass: "ranged",
        },
        {
            id: 3543,
            name: "Phantasm",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 50,
            weaponClass: "ranged",
        },

        // Hardmode Magic Weapons
        {
            id: 488,
            name: "Crystal Storm",
            mod: "vanilla",
            type: "weapon",
            rarity: 4,
            damage: 47,
            weaponClass: "magic",
        },
        {
            id: 1572,
            name: "Golden Shower",
            mod: "vanilla",
            type: "weapon",
            rarity: 4,
            damage: 21,
            weaponClass: "magic",
        },
        {
            id: 494,
            name: "Sky Fracture",
            mod: "vanilla",
            type: "weapon",
            rarity: 5,
            damage: 52,
            weaponClass: "magic",
        },
        {
            id: 1444,
            name: "Rainbow Gun",
            mod: "vanilla",
            type: "weapon",
            rarity: 7,
            damage: 45,
            weaponClass: "magic",
        },
        {
            id: 1446,
            name: "Razorpine",
            mod: "vanilla",
            type: "weapon",
            rarity: 8,
            damage: 69,
            weaponClass: "magic",
        },
        {
            id: 3541,
            name: "Last Prism",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 100,
            weaponClass: "magic",
        },
        {
            id: 3571,
            name: "Nebula Arcanum",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 60,
            weaponClass: "magic",
        },
        {
            id: 3570,
            name: "Nebula Blaze",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 60,
            weaponClass: "magic",
        },

        // Hardmode Summoner Weapons
        {
            id: 1309,
            name: "Spider Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 4,
            damage: 26,
            weaponClass: "summoner",
        },
        {
            id: 1157,
            name: "Optic Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 5,
            damage: 30,
            weaponClass: "summoner",
        },
        {
            id: 2584,
            name: "Deadly Sphere Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 7,
            damage: 50,
            weaponClass: "summoner",
        },
        {
            id: 2749,
            name: "Raven Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 8,
            damage: 55,
            weaponClass: "summoner",
        },
        {
            id: 3569,
            name: "Stardust Dragon Staff",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 40,
            weaponClass: "summoner",
        },
        {
            id: 3571,
            name: "Terraprisma",
            mod: "vanilla",
            type: "weapon",
            rarity: 10,
            damage: 90,
            weaponClass: "summoner",
        },

        // Armor Sets - Pre-Hardmode
        {
            id: 88,
            name: "Wood Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 0,
            defense: 1,
            armorType: "helmet",
        },
        {
            id: 89,
            name: "Wood Breastplate",
            mod: "vanilla",
            type: "armor",
            rarity: 0,
            defense: 1,
            armorType: "chestplate",
        },
        {
            id: 90,
            name: "Wood Greaves",
            mod: "vanilla",
            type: "armor",
            rarity: 0,
            defense: 1,
            armorType: "leggings",
        },

        {
            id: 2361,
            name: "Copper Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 0,
            defense: 2,
            armorType: "helmet",
        },
        {
            id: 2362,
            name: "Copper Chainmail",
            mod: "vanilla",
            type: "armor",
            rarity: 0,
            defense: 3,
            armorType: "chestplate",
        },
        {
            id: 2363,
            name: "Copper Greaves",
            mod: "vanilla",
            type: "armor",
            rarity: 0,
            defense: 2,
            armorType: "leggings",
        },

        {
            id: 122,
            name: "Shadow Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 2,
            defense: 7,
            armorType: "helmet",
        },
        {
            id: 123,
            name: "Shadow Scalemail",
            mod: "vanilla",
            type: "armor",
            rarity: 2,
            defense: 10,
            armorType: "chestplate",
        },
        {
            id: 124,
            name: "Shadow Greaves",
            mod: "vanilla",
            type: "armor",
            rarity: 2,
            defense: 8,
            armorType: "leggings",
        },

        {
            id: 793,
            name: "Molten Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 3,
            defense: 11,
            armorType: "helmet",
        },
        {
            id: 794,
            name: "Molten Breastplate",
            mod: "vanilla",
            type: "armor",
            rarity: 3,
            defense: 16,
            armorType: "chestplate",
        },
        {
            id: 795,
            name: "Molten Greaves",
            mod: "vanilla",
            type: "armor",
            rarity: 3,
            defense: 12,
            armorType: "leggings",
        },

        // Armor Sets - Hardmode
        {
            id: 551,
            name: "Cobalt Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 4,
            defense: 10,
            armorType: "helmet",
        },
        {
            id: 552,
            name: "Cobalt Breastplate",
            mod: "vanilla",
            type: "armor",
            rarity: 4,
            defense: 14,
            armorType: "chestplate",
        },
        {
            id: 553,
            name: "Cobalt Leggings",
            mod: "vanilla",
            type: "armor",
            rarity: 4,
            defense: 10,
            armorType: "leggings",
        },

        {
            id: 1001,
            name: "Adamantite Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 4,
            defense: 18,
            armorType: "helmet",
        },
        {
            id: 1002,
            name: "Adamantite Breastplate",
            mod: "vanilla",
            type: "armor",
            rarity: 4,
            defense: 23,
            armorType: "chestplate",
        },
        {
            id: 1003,
            name: "Adamantite Leggings",
            mod: "vanilla",
            type: "armor",
            rarity: 4,
            defense: 16,
            armorType: "leggings",
        },

        {
            id: 1001,
            name: "Hallowed Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 5,
            defense: 19,
            armorType: "helmet",
        },
        {
            id: 1002,
            name: "Hallowed Plate Mail",
            mod: "vanilla",
            type: "armor",
            rarity: 5,
            defense: 25,
            armorType: "chestplate",
        },
        {
            id: 1003,
            name: "Hallowed Greaves",
            mod: "vanilla",
            type: "armor",
            rarity: 5,
            defense: 18,
            armorType: "leggings",
        },

        {
            id: 1546,
            name: "Beetle Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 7,
            defense: 28,
            armorType: "helmet",
        },
        {
            id: 1547,
            name: "Beetle Scale Mail",
            mod: "vanilla",
            type: "armor",
            rarity: 7,
            defense: 34,
            armorType: "chestplate",
        },
        {
            id: 1548,
            name: "Beetle Leggings",
            mod: "vanilla",
            type: "armor",
            rarity: 7,
            defense: 24,
            armorType: "leggings",
        },

        {
            id: 2361,
            name: "Solar Flare Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 26,
            armorType: "helmet",
        },
        {
            id: 2362,
            name: "Solar Flare Breastplate",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 32,
            armorType: "chestplate",
        },
        {
            id: 2363,
            name: "Solar Flare Leggings",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 22,
            armorType: "leggings",
        },

        {
            id: 3551,
            name: "Vortex Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 20,
            armorType: "helmet",
        },
        {
            id: 3552,
            name: "Vortex Breastplate",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 30,
            armorType: "chestplate",
        },
        {
            id: 3553,
            name: "Vortex Leggings",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 18,
            armorType: "leggings",
        },

        {
            id: 3559,
            name: "Nebula Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 18,
            armorType: "helmet",
        },
        {
            id: 3560,
            name: "Nebula Breastplate",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 28,
            armorType: "chestplate",
        },
        {
            id: 3561,
            name: "Nebula Leggings",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 16,
            armorType: "leggings",
        },

        {
            id: 3567,
            name: "Stardust Helmet",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 14,
            armorType: "helmet",
        },
        {
            id: 3568,
            name: "Stardust Plate",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 24,
            armorType: "chestplate",
        },
        {
            id: 3569,
            name: "Stardust Leggings",
            mod: "vanilla",
            type: "armor",
            rarity: 10,
            defense: 12,
            armorType: "leggings",
        },

        // Accessories - Pre-Hardmode
        {
            id: 15,
            name: "Hermes Boots",
            mod: "vanilla",
            type: "accessory",
            rarity: 2,
        },
        {
            id: 211,
            name: "Cloud in a Bottle",
            mod: "vanilla",
            type: "accessory",
            rarity: 1,
        },
        {
            id: 158,
            name: "Band of Regeneration",
            mod: "vanilla",
            type: "accessory",
            rarity: 1,
        },
        {
            id: 289,
            name: "Magic Mirror",
            mod: "vanilla",
            type: "accessory",
            rarity: 1,
        },
        {
            id: 53,
            name: "Shackle",
            mod: "vanilla",
            type: "accessory",
            rarity: 1,
        },
        {
            id: 862,
            name: "Obsidian Shield",
            mod: "vanilla",
            type: "accessory",
            rarity: 3,
        },
        {
            id: 863,
            name: "Anklet of the Wind",
            mod: "vanilla",
            type: "accessory",
            rarity: 3,
        },

        // Accessories - Hardmode
        {
            id: 396,
            name: "Lightning Boots",
            mod: "vanilla",
            type: "accessory",
            rarity: 4,
        },
        {
            id: 898,
            name: "Frostspark Boots",
            mod: "vanilla",
            type: "accessory",
            rarity: 6,
        },
        {
            id: 3991,
            name: "Terraspark Boots",
            mod: "vanilla",
            type: "accessory",
            rarity: 7,
        },
        {
            id: 861,
            name: "Wings",
            mod: "vanilla",
            type: "accessory",
            rarity: 4,
        },
        {
            id: 1162,
            name: "Flame Wings",
            mod: "vanilla",
            type: "accessory",
            rarity: 5,
        },
        {
            id: 1165,
            name: "Fishron Wings",
            mod: "vanilla",
            type: "accessory",
            rarity: 8,
        },
        {
            id: 1866,
            name: "Mothron Wings",
            mod: "vanilla",
            type: "accessory",
            rarity: 7,
        },
        {
            id: 493,
            name: "Warrior Emblem",
            mod: "vanilla",
            type: "accessory",
            rarity: 4,
        },
        {
            id: 489,
            name: "Ranger Emblem",
            mod: "vanilla",
            type: "accessory",
            rarity: 4,
        },
        {
            id: 490,
            name: "Sorcerer Emblem",
            mod: "vanilla",
            type: "accessory",
            rarity: 4,
        },
        {
            id: 491,
            name: "Summoner Emblem",
            mod: "vanilla",
            type: "accessory",
            rarity: 4,
        },
        {
            id: 3998,
            name: "Celestial Shell",
            mod: "vanilla",
            type: "accessory",
            rarity: 8,
        },
        {
            id: 3095,
            name: "Celestial Stone",
            mod: "vanilla",
            type: "accessory",
            rarity: 7,
        },
        {
            id: 1613,
            name: "Ankh Shield",
            mod: "vanilla",
            type: "accessory",
            rarity: 7,
        },
        {
            id: 3034,
            name: "Charm of Myths",
            mod: "vanilla",
            type: "accessory",
            rarity: 6,
        },
        {
            id: 3035,
            name: "Frozen Turtle Shell",
            mod: "vanilla",
            type: "accessory",
            rarity: 7,
        },
        {
            id: 3061,
            name: "Celestial Emblem",
            mod: "vanilla",
            type: "accessory",
            rarity: 10,
        },
        {
            id: 3062,
            name: "Avenger Emblem",
            mod: "vanilla",
            type: "accessory",
            rarity: 7,
        },
        {
            id: 3064,
            name: "Destroyer Emblem",
            mod: "vanilla",
            type: "accessory",
            rarity: 8,
        },
        {
            id: 5000,
            name: "Celestial Cuffs",
            mod: "vanilla",
            type: "accessory",
            rarity: 6,
        },
        {
            id: 5001,
            name: "Fire Gauntlet",
            mod: "vanilla",
            type: "accessory",
            rarity: 7,
        },
        {
            id: 5002,
            name: "Mechanical Glove",
            mod: "vanilla",
            type: "accessory",
            rarity: 5,
        },

        // Tools
        {
            id: 3509,
            name: "Copper Pickaxe",
            mod: "vanilla",
            type: "tool",
            rarity: 0,
        },
        {
            id: 3506,
            name: "Gold Pickaxe",
            mod: "vanilla",
            type: "tool",
            rarity: 0,
        },
        {
            id: 103,
            name: "Molten Pickaxe",
            mod: "vanilla",
            type: "tool",
            rarity: 3,
        },
        { id: 579, name: "Drax", mod: "vanilla", type: "tool", rarity: 5 },
        { id: 990, name: "Picksaw", mod: "vanilla", type: "tool", rarity: 7 },
        {
            id: 3466,
            name: "Shroomite Digging Claw",
            mod: "vanilla",
            type: "tool",
            rarity: 8,
        },
    ]

    console.log(`✅ Generated ${items.length} vanilla items`)
    return items
}

async function main() {
    console.log("🚀 Generating vanilla items...\n")

    const vanillaItems = getVanillaItems()
    const vanillaPath = path.join(DATA_DIR, "items-vanilla.json")
    fs.writeFileSync(vanillaPath, JSON.stringify(vanillaItems, null, 2))
    console.log(`📝 Wrote ${vanillaPath}\n`)

    console.log(
        `✨ Done! Generated ${vanillaItems.length} vanilla items (IDs: 1 - 99,999)`
    )
}

main().catch(console.error)
